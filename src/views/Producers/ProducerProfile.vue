                                ...
                                <div id="carouselExample" class="carousel slide">
                                    <div class="carousel-inner ps-5 pe-5">
                                        ...
                                        <div class="carousel-item active">
                                            <p> <b> Q: {{ firstQuestion["question"] }} </b> </p>
                                            <p> {{ firstQuestion["answer"] }} </p>
                                        </div>
                                        <!-- [for users] other answered questions -->
                                        <div v-if="userType != 'producer'">
                                            <div  class="carousel-item" v-for="qa in restOfAnsweredQuestions" v-bind:key="qa">
                                                <p> <b> Q: {{ qa["question"] }} </b> </p>
                                                <p> {{ qa["answer"] }} </p>
                                            </div>
                                        </div>
                                        <div v-else>
                                            <div class="carousel-item" v-for="qa in restOfAllQuestions" v-bind:key="qa">
                                                <p> <b> Q: {{ qa["question"] }} </b> </p>
                                                answer here
                                            </div>
                                        </div>
    ...
    export default {
        data() {
            return {
                ...
                // to fetch producer's q&a
                producerAnswered: [],
                producerNotAnswered: [],
                firstQuestion: {},
                restOfAnsweredQuestions: [],
                restOfAllQuestions: [],
        ...
        methods: {
            ...
            // check if producer has answered questions or not
            async checkProducerAnswered() {
                let all_q_a = this.specified_producer["questionsAnswers"];
                for (let qa in all_q_a) {
                    let question = all_q_a[qa]["question"];
                    let answer = all_q_a[qa]["answer"];
                    // if producer has answered questions
                    if (answer != "") {
                        this.producerAnswered.push({
                            question: question,
                            answer: answer
                        })
                    }
                    // if producer has not answered questions
                    else {
                        this.producerNotAnswered.push({
                            question: question,
                            answer: answer
                        })
                    }
                }
                // get first & rest of the answered question that producer answered
                if (this.producerAnswered.length > 0) {
                    // first question (display)
                    let firstQuestion = this.producerAnswered[0];
                    this.firstQuestion["question"] = firstQuestion["question"];
                    this.firstQuestion["answer"] = firstQuestion["answer"];
                    // [for users] other answered questions
                    let restOfAnsweredQuestions = this.producerAnswered.slice(1);
                    for (let qa in restOfAnsweredQuestions) {
                        let question = restOfAnsweredQuestions[qa]["question"];
                        let answer = restOfAnsweredQuestions[qa]["answer"];
                        this.restOfAnsweredQuestions.push({
                            question: question,
                            answer: answer
                        })
                    }
                    // [for producers other] all other questions
                    let unansweredQuestions = this.producerNotAnswered;
                    for (let qa in unansweredQuestions) {
                        let question = unansweredQuestions[qa]["question"];
                        let answer = unansweredQuestions[qa]["answer"];
                        this.restOfAllQuestions = this.restOfAnsweredQuestions
                        this.restOfAllQuestions.push({
                            question: question,
                            answer: answer
                        })
                    }
